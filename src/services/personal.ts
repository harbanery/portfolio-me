...
/**
 * Data-cache profile for this service layer. The pages render dynamically
 * (nonce CSP), so database freshness is preserved here instead: every
 * query is wrapped in `unstable_cache` with a 60-second revalidate — the
 * same cadence the pages' former ISR `revalidate = 60` gave. Concurrent
 * renders inside the window share one result and never touch Prisma.
 */
 ...
 */
const getExperienceStatsCached = unstable_cache(
  async (): Promise<ExperienceStats> => {
    ...
    return {
      companies,
      years: months > 0 ? Math.max(1, Math.round(months / 12)) : 0,
    };
  },
  ["experience-stats"],
  DATA_CACHE,
...
const getPersonalProfileCached = unstable_cache(
  async (): Promise<PersonalProfile | null> => {
    try {
...
export async function getPersonalProfile(): Promise<PersonalProfile | null> {
  return getPersonalProfileCached();
}
...
const getPersonalContactEmailCached = unstable_cache(
...
export async function getPersonalContactEmail(): Promise<string | null> {
  return getPersonalContactEmailCached();
}
...
const getExperiencesCached = unstable_cache(
  async (): Promise<ExperienceTimelineEntry[]> => {
