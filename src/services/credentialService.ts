 ...
 *
 * Queries are wrapped in `unstable_cache` (60s revalidate — the cadence
 * the pages' former ISR gave) so concurrent dynamic renders share one
 * result and skip Prisma.
 */
...
const getPrimaryCvCached = unstable_cache(
  async (): Promise<CvFile | null> => {
    try {
...
export async function getPrimaryCv(): Promise<CvFile | null> {
  return getPrimaryCvCached();
}

const getEducationCached = unstable_cache(
  async (): Promise<EducationItem[]> => {
    try {
...
export async function getEducation(): Promise<EducationItem[]> {
  return getEducationCached();
}

const getCredentialsCached = unstable_cache(
  async (): Promise<CredentialItem[]> => {
    try {
